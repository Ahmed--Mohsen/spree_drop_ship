        ...
        def split(packages)
          ...
          packages.each do |package|
            ...
            drop_ship.each do |content|
              ...
              suppliers = content.variant.supplier_variants.order('spree_supplier_variants.cost ASC').map(&:supplier)
              # Select first supplier that has stock location with avialable stock item.
              available_supplier = suppliers.detect { |supplier| supplier.stock_locations_with_available_stock_items.any? }
